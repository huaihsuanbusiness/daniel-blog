...
這篇會用一個很實際的案例來談這件事，也就是求職 agent 裡的 JD、CV、rubric 檢索。這個場景很適合講 chunking，因為它不是單純問答，而是很吃對應關係、可引用性、以及輸出穩定度的工作流。你很快就會看到，CV 跟 rubric 根本不該用同一把刀切。fileciteturn13file6turn13file16
...
如果它對第一件事有利，對第二件事卻很糟，那它就不是好 chunk。fileciteturn13file6
...
只要 JD 提到 AI，這整塊都可能被抓回來。但模型真正需要的，搞不好只有跟 AI prototype 有關的兩句。剩下的 scale metrics 跟 partnership 只是讓 relevant reason 變得更吵。fileciteturn13file18
...
這就是為什麼「越細越好」這種說法很危險。不是不能切細，而是要看你到底在切什麼。fileciteturn13file6
...
而對應關係問題最怕的，就是一塊資料裡混了太多不同訊號。只命中一個，卻整盤端回來。fileciteturn13file18
...
這樣抓回來的 topK，比較像「證據清單」，不是「自傳章節」。fileciteturn13file16turn13file18
...
這種漂移很煩，因為它看起來不像大 bug，卻會慢慢把系統穩定性磨掉。fileciteturn13file16
...
你把 rubric 從 9 段調成 11 段，我覺得就是這個方向。不是為了追求更碎，而是讓每段更自足。fileciteturn13file4turn13file16
...
這個配方的重點不是它神奇，而是它讓 evidence 保持在一個模型還吃得下、也比較不會飄的範圍裡。比起盲目加 topK，這種做法通常更省 tokens，也比較穩。fileciteturn13file4turn13file7turn13file16
