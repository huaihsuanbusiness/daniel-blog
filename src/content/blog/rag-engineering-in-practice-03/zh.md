...
Pinecone 與 LangChain 的文件都會談 chunking 的起跑方式。LangChain 也很直接地建議，多數情況可以先從 `RecursiveCharacterTextSplitter` 起跑，再去調 `chunk_size` 跟 `chunk_overlap`。這些都很合理。 但合理的起跑點，不等於完整的設計原則。
...
- **程式碼或設定檔**：常常更適合結構感知或語言感知邊界 所以真正該先問的，不是「多少 tokens」，而是：
