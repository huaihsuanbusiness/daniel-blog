...
這也說明 Part 02 / Part 03 的 14 站應該怎麼讀：每個 mode 啟用的是 capability map 的不同子集合。`fast` 會刻意跳過大多數 verification 與 context-expansion 站點；`safe` 會打開 grounded document QA 需要的站點；`deep_eval` 則多開昂貴的 evaluation 站點做 debug / regression。地圖定義可用能力，router 決定 runtime path。
