...
1. filename: tracking-plan-data-contract-stack.svg
   purpose: 用四層結構圖表現 business purpose、event、property、QA 之間的關係
   placement: 放在「Tracking plan 真正該長什麼樣子」段落後
   alt: Tracking plan 作為資料合約的四層結構圖
...
2. filename: booking-funnel-closure-map.svg
   purpose: 示意訂房產品的 funnel closure 與關鍵 join keys
   placement: 放在「漏斗閉合其實是 tracking plan 最常被忽略的驗收」段落後
   alt: 訂房產品漏斗閉合與關鍵識別碼示意圖
