...
description: "demo 階段 RAG 跑得起來、答案看起來合理，但 production 真正死點是「看起來對、其實不對」——本文拆解 3 個 production 級能力：可量測（faithfulness / citation check + RAGAS）、可除錯（Langfuse tracing query-level + stage-level）、可控（deterministic-first + async evaluator + cost tracking），含 Step 1-7 建構順序、6 件 production launch checklist、5 種 query mode 銜接 Part 08。Faithfulness / Citation check + RAGAS offline eval + Langfuse tracing 是業界 RAG 評估的標準流程（從 RAGAS、Langfuse、DeepEval 官方文件可對齊）。"
categories: ["ai"]
tags: ["ai", "rag", "production-rag", "llamaindex", "evaluation", "faithfulness", "citation", "langfuse", "ragas", "observability"]
...
真實 query 跑出來長這樣（faithfulness 抓出 60 天通知的編造 claim）：
...
```json
{
  "faithfulness_check": {
    ...
    "passed": false,
    "claims": [
      {"claim": "early termination 違約金是 6 個月訂閱費", "supported": true, "supporting_sources": ["[2]"], "reason": "Section 3.3 明寫"},
      {"claim": "early termination 需 60 天书面通知", "supported": true, "supporting_sources": ["[2]"], "reason": "Section 3.3 明寫 60 days written notice"}
    ],
    "warnings": []
  },
  "citation_check": {
    "score": 0.92,
    "passed": true,
    "claim_citations": [{"claim": "違約金 6 個月訂閱費", "cited_markers": ["[2]"], "supported_markers": ["[2]"], "missing_citation": false, "support_score": 1.0}]
  },
  "observability": {
