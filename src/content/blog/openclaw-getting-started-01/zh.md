...
1. filename: openclaw-01-architecture-boundary.svg  
   purpose: 說明 Gateway、Agent、Channel、Tools、Skills、Memory、Node 的責任分層  
   placement: 放在「最小心智模型：七個元件先切清楚」段落後  
   alt: OpenClaw 的核心元件與責任邊界圖  
   prompt: A clean blog-friendly SVG architecture diagram showing OpenClaw Gateway at the centre, with Agent, Channels, Tools, Skills, Memory, and optional macOS Node around it. Use modern rounded rectangles, soft colours, clear arrows, English labels inside the figure, and avoid clutter.

2. filename: openclaw-01-deployment-map.svg  
   purpose: 比較 Mac mini、Docker、VM、VPS 四種部署形態  
   placement: 放在「部署形態比較」段落後  
   alt: OpenClaw 的四種常見部署形態比較圖  
   prompt: A blog-friendly SVG comparing Mac mini, Docker, VM, and VPS as OpenClaw deployment options. Show tradeoffs in isolation, convenience, and operational complexity. Minimalist style, English labels, table-like or quadrant layout.
