...
| Part | Status | Title |
|---|---|---|
| **Part 1** | Published | A six-dimensional map of LLM agent architectures |
| **Part 2** | Current page ✅ | Agent execution paths explained: Direct, Pipeline, Router, State Machine and DAG |
| Part 3 | Drafting | (To be added when Part 3 ships) |
| Subsequent parts | Planning | (to be defined) |

Two parts are currently published. This table only marks positions already published or planned; the full in-series navigation, total count and current-position indicator are generated by the page template from actual publishing state (source of truth: `seriesMeta.ts`).